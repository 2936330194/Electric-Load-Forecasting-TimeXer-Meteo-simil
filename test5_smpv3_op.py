"""
Optuna hyperparameter tuning for test5_smpv3.py.
"""
...
FIX_SEED = 2026
DEFAULT_CHECKPOINTS_ROOT = "./ck5o/"
DEFAULT_RESULTS_ROOT = "./rs5o/"
DEFAULT_OPTUNA_OUTPUT_ROOT = "./op5/"
DEFAULT_OPTUNA_STUDY_NAME = "t5v3"
...
def _base_args_dict(weather_source: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "is_training": 1,
        "model_id": f"{core.MODEL_ID_PREFIX}_sdv3",
        "model": core.MODEL,
        "des": core.DES,
        "itr": core.ITR,
        "data": "custom",
...
def train_quantile_model(model, args, device, weather_store: task.WeatherGridStore, trial=None):
    ...
    if bool(getattr(args, 'use_similar_day_prior', False)):
        print(
            f"similar_day_top_k: {getattr(args, 'similar_day_top_k', 0)} "
...
def run_experiment(
    ...
    *,
    ...
    try:
        ...
        model = task.FullMapConvTimeXerSimilarTokenQuantile(args, quantiles=args.quantiles).float().to(device)
...
def build_cli_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Optuna hyperparameter tuning for test5_smpv3.py")
    parser.add_argument(
        "--weather-source",
        type=str,
