"""
新版 test4_smp.py 的 Optuna 超参数寻优入口。
"""
...
FIX_SEED = 2026
DEFAULT_CHECKPOINTS_ROOT = "./checkpoints_quantile_optuna/"
DEFAULT_RESULTS_ROOT = "./results_optuna/"
DEFAULT_OPTUNA_OUTPUT_ROOT = "./optuna_runs/"
DEFAULT_OPTUNA_STUDY_NAME = "test4_smp_optuna_v2"
...
USE_BEST_TRIAL_FILE = "best_trial_result.json"

DEFAULT_OPTUNA_SEARCH_SPACE: Dict[str, Any] = {
    "WEATHER_FEATURE_DIM": {"type": "categorical", "choices": [2, 3, 4, 5, 6, 7, 8]},
    "D_MODEL": {"type": "categorical", "choices": [128, 256, 384, 512]},
    "N_HEADS": {"type": "categorical", "choices": [2, 4, 8, 12]},
    "E_LAYERS": {"type": "int", "low": 2, "high": 3, "step": 1},
    "D_FF": {"type": "categorical", "choices": [256, 512, 1024, 1536, 2048]},
    "DROPOUT": {"type": "float", "low": 0.05, "high": 0.20, "step": 0.05},
    "PATCH_LEN": {"type": "categorical", "choices": [24, 48, 72, 96]},
    "BATCH_SIZE": {"type": "categorical", "choices": [32, 48, 64, 72, 80]},
...
def _base_args_dict() -> Dict[str, Any]:
    return {
        ...
        "weather_h5_specs": base.WEATHER_H5_SPECS,
        ...
        "weather_seq_len": getattr(base, "WEATHER_SEQ_LEN", base.SEQ_LEN + base.PRED_LEN),
...
def _build_args(train_mode: bool = True, overrides: Optional[Dict[str, Any]] = None) -> argparse.Namespace:
    config = _base_args_dict()
    config["is_training"] = 1 if train_mode else 0
    config.update(_normalize_param_overrides(overrides))

    int_fields = [
        "seq_len", "label_len", "pred_len", "enc_in", "c_out", "d_model", "n_heads",
        "e_layers", "d_ff", "factor", "patch_len", "use_norm", "weather_in_channels",
        "weather_feature_dim", "weather_grid_height", "weather_grid_width",
        "weather_kernel_height", "weather_kernel_width", "weather_encode_chunk_size",
        "weather_seq_len", "num_workers", "itr", "train_epochs", "batch_size",
        "patience", "gpu", "n_quantiles",
    ]
    for field in int_fields:
        config[field] = int(config[field])
    for field in ["dropout", "learning_rate"]:
        config[field] = float(config[field])
    for field in ["pin_memory", "contiguous_train_batches", "use_amp", "inverse_eval", "use_gpu", "use_multi_gpu"]:
    ...
    if config["patch_len"] <= 0 or config["patch_len"] > config["seq_len"]:
        raise ValueError("patch_len 必须位于 (0, seq_len]。")
    if config["weather_seq_len"] < config["seq_len"]:
        raise ValueError("weather_seq_len 不能小于 seq_len。")
...
def _normalize_search_spec(spec: Any) -> Dict[str, Any]:
    ...
    if not isinstance(spec, dict):
        raise ValueError(f"非法搜索空间配置: {spec}")
...
def _suggest_from_spec(trial, name: str, spec: Any) -> Any:
    ...
    if spec_type == "float":
        ...
        if log:
            raise ValueError(f"{name} 不能同时设置 step 和 log。")
        return trial.suggest_float(name, low, high, step=float(step))
    raise ValueError(f"不支持的搜索空间类型: {name} -> {spec_type}")


def _load_search_space(search_space_json: Optional[str], search_space: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("search_space_json 必须是 JSON 对象。")
    return payload
...
def _sample_trial_params(trial, search_space: Dict[str, Any], base_args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if unknown_keys:
        raise ValueError(f"存在不支持的调参字段: {unknown_keys}")

    sampled: Dict[str, Any] = {}
    discrete_int_keys = {"WEATHER_FEATURE_DIM", "D_MODEL", "N_HEADS", "E_LAYERS", "D_FF", "PATCH_LEN", "BATCH_SIZE"}
    ...
    if current_d_model % current_n_heads != 0:
        raise TrialPruned(f"非法注意力配置: d_model={current_d_model}, n_heads={current_n_heads}")
    return sampled


def train_quantile_model(model, args, device, weather_store: WeatherGridStore, trial=None):
    ...
    print(
        f"weather_seq_len: {getattr(args, 'weather_seq_len', args.seq_len)} "
    ...
    for epoch in range(args.train_epochs):
        ...
        for i, (
            ...
            if not torch.isfinite(outputs).all():
                message = f"模型输出出现非有限值: epoch={epoch + 1}, iter={i + 1}"
                if trial is not None:
                    raise TrialPruned(message)
                raise RuntimeError(message)
            if not torch.isfinite(loss):
                message = f"训练损失出现非有限值: epoch={epoch + 1}, iter={i + 1}"
                if trial is not None:
                    raise TrialPruned(message)
                raise RuntimeError(message)
        ...
        vali_loss = base.validate_quantile(model, vali_loader, criterion, args, device, use_amp=use_amp)
        ...
        if trial is not None:
            if not np.isfinite(vali_loss):
                raise TrialPruned("验证损失不是有限值。")
            trial.report(float(vali_loss), step=epoch + 1)
            if trial.should_prune():
                raise TrialPruned(f"Trial 在 epoch {epoch + 1} 被剪枝, vali_loss={vali_loss:.7f}")
    ...
    if not os.path.exists(best_model_path):
        raise FileNotFoundError(f"最优 checkpoint 不存在: {best_model_path}")
    model.load_state_dict(torch.load(best_model_path, map_location=device))
    print(f"Loaded best model weights: {best_model_path}")
    return model, float(best_vali_loss)


def test_quantile_model(model, args, device, weather_store: WeatherGridStore) -> Dict[str, Any]:
    ...
    use_amp = bool(getattr(args, "use_amp", False)) and device.type == "cuda"
    use_non_blocking = base._use_non_blocking_transfer(args, device)
    p50_idx = base.QUANTILES.index(0.5)
...
def run_experiment(
    ...
    *,
    ...
    setting = base._get_setting(args)
    result: Dict[str, Any] = {"setting": setting}
    ...
    try:
        ...
        if args.is_training:
            ...
            model, best_vali_loss = train_quantile_model(
                model,
                args,
                device,
                weather_store,
...
def _export_best_trial_to_use(
    ...
    print("\n" + "=" * 72)
    print(f"Best trial exported to /use: {use_dir}")
...
def run_optuna_study(cli_args) -> Dict[str, Any]:
    ...
    base_args = _build_args(train_mode=True, overrides=runtime_overrides)
    ...
    def objective(trial) -> float:
        ...
        args = _build_args(train_mode=True, overrides=trial_overrides)
        setting = base._get_setting(args)
        checkpoint_path = os.path.join(args.checkpoints, setting, "checkpoint.pth")
    ...
    try:
        ...
        raise RuntimeError("Optuna 未产生可用的完成试验，无法导出最优结果。") from exc
    ...
    best_args = _build_args(train_mode=False, overrides=best_overrides)
...
def build_cli_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Optuna hyperparameter tuning for upgraded test4_smp.py")
