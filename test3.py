"""
test3.py - Simple Full-Map Conv + TimeXer 端到端训练
...
CHECKPOINTS_DIR = "./checkpoints_test3/" # checkpoints文件夹
...
def _configure_runtime_weather_args(
    ...
    这样 1h 数据会自动得到 168+24，15min 数据会自动得到 672+96。
    """
    ...
    args.weather_future_len = int(weather_future_len)
    args.weather_seq_len = int(weather_seq_len)
    args.model_id = (
        f"{MODEL_ID_PREFIX}_{weather_source}_{weather_step_freq}Wx_"
        f"wh{args.weather_history_len}_wf{args.weather_future_len}_"
        f"pl{args.pred_len}_FullMapConv_Exo"
    )

    print(
        "[weather-config] "
        f"source={weather_source}, step={weather_step_freq}, "
        f"history={args.weather_history_len}, future={args.weather_future_len}, "
