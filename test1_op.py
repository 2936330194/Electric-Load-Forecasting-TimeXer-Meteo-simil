"""
Optuna hyperparameter tuning for test1.py.
"""
...
FIX_SEED = 2026
DEFAULT_CHECKPOINTS_ROOT = "./checkpoints_test1_optuna/"
DEFAULT_RESULTS_ROOT = "./results_test1_optuna/"
DEFAULT_OPTUNA_OUTPUT_ROOT = "./optuna_runs/"
DEFAULT_OPTUNA_STUDY_NAME = "test1_optuna_v1"
DEFAULT_OPTUNA_N_TRIALS = 128
DEFAULT_OPTUNA_TIMEOUT = 0
DEFAULT_OPTUNA_DIRECTION = "minimize"

DEFAULT_USE_DIR = "./optuna"
USE_BEST_PARAMS_FILE = "best_params1.json"
USE_BEST_CONFIG_FILE = "best_config1.json"
USE_BEST_WEIGHT_FILE = "best_model1.pth"
USE_BEST_TRIAL_FILE = "best_trial_result1.json"
...
def _build_args(train_mode: bool = True, overrides: Optional[Dict[str, Any]] = None) -> argparse.Namespace:
    ...
    for field in ["use_amp", "inverse_eval", "use_gpu", "use_multi_gpu"]:
...
def _sample_trial_params(trial, search_space: Dict[str, Any], base_args: argparse.Namespace) -> Dict[str, Any]:
    ...
    discrete_int_keys = {"D_MODEL", "N_HEADS", "E_LAYERS", "D_FF", "PATCH_LEN", "BATCH_SIZE"}
...
def train_quantile_model(model, args, device, trial=None):
    _, train_loader = base.data_provider(args, "train")
    _, vali_loader = base.data_provider(args, "val")
    ...
    criterion = base.SharedQuantileLoss(args.quantiles)
    early_stopping = base.EarlyStopping(patience=args.patience, verbose=True)

    best_vali_loss = np.inf
    print("\n" + "=" * 72)
    print("Start Optuna trial training: TimeXer Quantile")
    ...
    for epoch in range(args.train_epochs):
        ...
        vali_loss = float(base.validate_quantile(model, vali_loader, criterion, args, device))
...
def test_quantile_model(model, args, device) -> Dict[str, Any]:
    test_data, test_loader = base.data_provider(args, "test")
    ...
    with torch.no_grad():
        for batch_x, batch_y, batch_x_mark, batch_y_mark in test_loader:
            ...
            f_dim = -1 if args.features == "MS" else 0
            batch_y_target = batch_y[:, -args.pred_len :, f_dim:]
            p50_pred = outputs[:, :, base.P50_IDX : base.P50_IDX + 1]
    ...
    if test_data.scale:
        ...
        for qi in range(args.n_quantiles):
            ...
            q_inv = test_data.inverse_transform(q_slice.reshape(q_shape[0] * q_shape[1], -1)).reshape(
...
def run_experiment(
    ...
    *,
    ...
    try:
        model = base.TimeXerQuantile(args, quantiles=args.quantiles).float().to(device)
        total_params = sum(p.numel() for p in model.parameters())
        print(f"TimeXerQuantile total params: {total_params:,}")
        ...
        if args.is_training:
            ...
            model, best_vali_loss = train_quantile_model(model, args, device, trial=trial)
        ...
        if run_test:
            ...
            eval_result = test_quantile_model(model, args, device)
            result.update(eval_result)
            results_dir = eval_result["results_dir"]

            if plot_results:
                base.shared_plot_pred_vs_true(
                    results_dir,
            ...
            if predict_future:
                base.shared_predict_future_load_from_csv(
                    model=model,
                    args=args,
                    device=device,
                    weather_store=None,
                    ...
                    use_inverse=args.inverse_eval,
                    quantiles=args.quantiles,
                    data_provider_fn=base.data_provider,
                    model_label="TimeXer",
                )
...
def build_cli_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Optuna hyperparameter tuning for test1.py")
