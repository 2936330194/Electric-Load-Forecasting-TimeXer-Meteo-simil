...
class InformerQuantile(nn.Module):
    def __init__(self, configs, quantiles=None):
        super(InformerQuantile, self).__init__()
        self.quantiles = quantiles if quantiles is not None else QUANTILES
        self.n_quantiles = len(self.quantiles)
        self.informer = Informer(configs)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        point_pred = self.informer(x_enc, x_mark_enc, x_dec, x_mark_dec, mask)
        point_pred = point_pred[:, -self.informer.pred_len:, :]
...
def train_quantile_model(model, args, device):
    ...
    print(f"\n{'='*60}")
    print(f"开始训练 Informer-Quantile 模型")
...
def main():
    ...
    model = InformerQuantile(args, quantiles=QUANTILES).float().to(device)
    total_params = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"InformerQuantile total params: {total_params:,}")
    print(f"InformerQuantile trainable params: {trainable_params:,}")
    ...
    metrics_path = os.path.join(args.checkpoints, "informer_metrics.json")
