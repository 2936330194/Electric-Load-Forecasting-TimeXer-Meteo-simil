...
class DLinearQuantile(nn.Module):
    def __init__(self, configs, quantiles=None):
        super(DLinearQuantile, self).__init__()
        self.quantiles = quantiles if quantiles is not None else QUANTILES
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        point_pred = self.dlinear(x_enc, x_mark_enc, x_dec, x_mark_dec, mask)
        point_pred = point_pred[:, -self.dlinear.pred_len:, :]
...
def train_quantile_model(model, args, device):
    ...
    print(f"\n{'='*60}")
    print(f"开始训练 DLinear-Quantile 模型")
...
def plot_pred_vs_true(results_dir, use_inverse=False):
    ...
    if np.isfinite(mape_val):
        plt.title(f"DLinear Quantile Prediction - MAPE: {100*mape_val:.2f}%")
    else:
        plt.title("DLinear Quantile Prediction - MAPE: NaN")
...
def predict_future_load_from_csv(model, args, device, results_dir, future_path, steps=96, use_inverse=True):
    ...
    history_window = history_values[-args.seq_len:].copy()

    model.eval()
    with torch.no_grad():
        batch_x = torch.as_tensor(history_window, dtype=torch.float32, device=device).unsqueeze(0)
    ...
    plt.grid(True, linestyle="--", alpha=0.5)
    plt.title(f"DLinear Future {predict_steps}-Step Load Prediction (Quantile)")
...
def main():
    parser = argparse.ArgumentParser(description="DLinear Baseline")
    ...
    metrics_path = os.path.join(args.checkpoints, "dlinear_metrics.json")
